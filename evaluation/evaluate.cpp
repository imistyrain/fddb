	...
	if(showMatchPairs)
	{
	  if(mps!= NULL)
	  {
	    ...
	    for(unsigned int i=0; i< mps->size(); i++)
	    {
	      ...
	      mask = ( (EllipseR *)(mps->at(i)->r1) )->display(mask, cv::Scalar(255,0,0), 3, textDetInd);
	      switch(detFormat)
	      {
		case(DET_RECTANGLE):
		  mask = ( (RectangleR *)(mps->at(i)->r2) )->display(mask, cv::Scalar(0,255,0), 3, textDetInd);
		  break;
		case(DET_ELLIPSE):
		  mask = ( (EllipseR *)(mps->at(i)->r2) )->display(mask, cv::Scalar(0,255,0), 3, textDetInd);
		  break;
		case(DET_PIXELS):
		  cerr << "Not yet implemented " << endl;
		  return -1;
